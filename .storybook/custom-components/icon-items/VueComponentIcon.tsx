import React from 'react';
import Tippy from '@tippyjs/react';
import 'tippy.js/dist/tippy.css';
import { ComponentIconProps } from './types.ts';

export function VueComponentIcon({ tooltipPosition = 'bottom', asLink = true, tooltip = 'Vue-enhanced component' }: ComponentIconProps) {
	const icon = (
		<svg version="1.1" viewBox="0 0 261.76 226.69" xmlns="http://www.w3.org/2000/svg">
			<g transform="matrix(1.3333 0 0 -1.3333 -76.311 313.34)">
				<g transform="translate(178.06 235.01)">
					<path d="m0 0-22.669-39.264-22.669 39.264h-75.491l98.16-170.02 98.16 170.02z" fill="#41b883"/>
				</g>
				<g transform="translate(178.06 235.01)">
					<path d="m0 0-22.669-39.264-22.669 39.264h-36.227l58.896-102.01 58.896 102.01z" fill="#34495e"/>
				</g>
			</g>
		</svg>);

	return asLink ? (
		<Tippy content={tooltip} placement={tooltipPosition}>
			<a className="sbdocs__icon__inner sbdocs__icon__inner--vue"
				href="https://cometcomponents.io/docs/technical-deep-dives/js-architecture/vue.html"
				target="_blank"
			>
				{icon}
			</a>
		</Tippy>
	) : (
		<Tippy content={tooltip} placement={tooltipPosition}>
			<span tabIndex={0} className="sbdocs__icon__inner sbdocs__icon__inner--vue">
				{icon}
			</span>
		</Tippy>
	);
}
