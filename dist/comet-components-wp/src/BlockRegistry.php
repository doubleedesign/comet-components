...
class BlockRegistry extends JavaScriptImplementation {
	...
	function override_core_block_rendering(): void {
		...
		foreach ($core_blocks as $core_block) {
			...
			if ($block_type) {
				...
				$settings = ['title' => $block_type->title, 'description' => $block_type->description, 'category' => $block_type->category, 'icon' => $block_type->icon, 'keywords' => $block_type->keywords, 'supports' => $block_type->supports, 'attributes' => $block_type->attributes, 'uses_context' => $block_type->uses_context, 'provides_context' => $block_type->provides_context, 'example' => $block_type->example, 'api_version' => $block_type->api_version, 'textdomain' => $block_type->textdomain, 'render_callback' => self::render_block_callback($core_block)];
	 ...
	 */
	static function render_block($block_name, $attributes, $content, $block_instance): string {
