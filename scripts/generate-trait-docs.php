...
class TraitDocGenerator {
    ...
    private function generateMarkdown(string $traitName): string {
        ...
        $propertiesOutput = implode('', array_map(fn(ReflectionProperty $property) => $this->generate_definition_item($property), $properties));
        $methodsOutput = implode('', array_map(fn(ReflectionMethod $method) => $this->generate_definition_item($method), $methods));
