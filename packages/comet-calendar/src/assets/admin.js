	...
	if(document.body.classList.contains('post-type-event') && document.body.classList.contains('edit-php')) {
		...
		forms.forEach(function (form) {
			form.addEventListener('submit', function (event) {
				...
				// Custom flag to identify the request for PHP processing
							...
							Object.entries(response.data.fields).forEach(([key, value]) => {
								...
								if (text) {
									if(typeof value === 'string') {
										text.innerHTML = formattedData(value);
									}
									if(typeof value === 'object') {
										if(Object.values(value).length === 2) {
											text.innerHTML = Object.values(value)
												.map(val => formattedData(val))
												.join(' - ');
										}
										else {
											text.innerHTML = Object.values(value).map(val => {
												if(typeof val === 'string') {
													return formattedData(val);
												}
												if(typeof val === 'object') {
													return Object.values(val).map(v => formattedData(v))
														.filter(v => v !== null)
														.join('<br>');
												}
											}).join('<br> ');
...
function formattedData(value) {
	...
	if(typeof value === 'object') {
		return Object.values(value).map(val => formattedData(val)).join('<br>');
	}
