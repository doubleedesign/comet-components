...
| `comet_blocks_related_content_card_list_card_as_link`                  | 	  `bool $as_link` `string $blockName`  | Set whether the cards in the various related content blocks render as links. Default is `false`, which renders them with a "Read more" button link. Alter behaviour differently for different blocks using the `$blockName` argument.                                                                                          |
...
| `comet_blocks_related_content_card_list_behaviour_when_fewer_than_max` | `string $behaviour` `string $blockType` | Set the behaviour of the related content blocks  when there are fewer items than the maximum per row in grid format. Options are `default` (keep the width of the cards as though the max is there), and `expand`. Use the `$blockType` argument to have different behaviour for different blocks that this filter applies to. |
...
| `comet_canvas_blog_post_include_author_card`    | `bool $include`       | Set whether to include the author bio at the bottom of single blog posts. Default is `false`.                                            |
| `comet_canvas_blog_post_include_post_nav`       | `bool $include`       | Set whether to include post navigation links at the bottom of single blog posts. Default is `true`.                                      |
| `comet_canvas_blog_post_featured_image_classes` | `array $classes`      | Add CSS class(es) to the wrapper for the featured image in single blog posts, e.g., `['breakout']`.                                      |
| `comet_canvas_blog_post_featured_image_style`   | `string $style`       | Set the `data-style` attribute for the featured image in single blog posts. Commonly supported values include `polaroid` and `rounded`.  |
